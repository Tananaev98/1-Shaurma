...
//--------Делаем дополнительный запрос к бд и выводим список и количество ингредиентов для каждого объекта меню-------------------------------- 
$sql1 = "select r.name, rr.quant_reserv
from menu m 
join requirement_reserves rr on m.id = rr.id_menu  
join reserves r on rr.id_reserves = r.id
where m.id = $id_object";//Подготавливаем запрос для вывода потребности ингредиентов на объект меню
$result1 = pg_query($conn, $sql1);//выполняем запрос к бд используя подготовленное соединение и запрос

echo "<div class = 'reserve'>"; //создаем блок с классом для возможности стилизации 

while ($row1 = pg_fetch_assoc($result1)) {//вложенный цикл для вывода наименований и количества ингредиентов
    
    $reserves_name=$row1["name"];//сохраняем результат перебора в переменную 
    $reserves_quant=$row1["quant_reserv"];//сохраняем результат перебора в переменную
    
    echo"$reserves_name: $reserves_quant гр.<br>";//выводим наименование и количество 
