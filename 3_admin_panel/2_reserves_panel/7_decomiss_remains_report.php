...
(rr.cost_gram * (rr.quant_received - rr.quant_spent)) AS summ_d

FROM reserves r
INNER JOIN remains_reserves rr ON r.id = rr.id_reserves
WHERE rr.quant_received - rr.quant_spent > 0
AND EXTRACT(DAY FROM (CAST(rr.expiration_date AS TIMESTAMP) - CAST(rr.date_of_receipt AS TIMESTAMP))) <= 0;";//Подготавливаем запрос по поиску соответсвия логина и пароля в базе. используем обратную сортировку для того что бы
...
<th>Дата списания</th>
<th>Количество списанного товара</th>
<th>Сумма убытков</th>
...
while ($row = pg_fetch_assoc($result)) { //Задаем условаие и начинаем цикл, цикл будет продолжаться до тех пор пока не будут перебраны все записи запроса
    ...
    echo "<td>" . $row['summ_d'] . "</td>";//ячейка таблицы
