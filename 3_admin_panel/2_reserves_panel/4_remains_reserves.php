...
<style>
        ...
        .table_reserves { /* Стили для таблицы вывода запасов*/
            margin: 0 auto; /* Устанавливаем автоматические отступы слева и справа, чтобы центрировать таблицу */
...
$sql = "select rr.id, r.name, rr.date_of_receipt, rr.cost_gram, rr.quant_received, rr.quant_spent, rr.expiration_date, rr.status 
FROM reserves r
INNER JOIN remains_reserves rr ON r.id = rr.id_reserves
WHERE rr.quant_received - rr.quant_spent > 0
ORDER BY rr.status desc;";//Подготавливаем запрос по поиску соответсвия логина и пароля в базе. используем обратную сортировку для того что бы
...
echo "<a class='add_object' href='4.1_remains_reserves.php'><img src='1_image/1_Add.png' class='imagesh'></a>";//выводим изображение ссылку на создание объекта
...
<th>Цена(за 1 грам)</th>
<th>Количество поступления(в граммах)</th>
<th>Расход(в граммах)</th>
<th>Дата истечения срока</th>
<th>Разрешение на использование</th>
...
//---------------------------Условие которое задает цвет колонке в зависимости от значения
$columnValue = $row["status"]; // Получаем значение второй колонки из базы данных

if ($columnValue === "Разрешен") {// Определяем цвет текста в зависимости от значения второй колонки
    $color = "green"; // Зеленый цвет для "Разрешен"
} else {
    $color = "red"; // Красный цвет для других значений
}
//----------------------------------------------------------------------------------------
$id_object = $row['id']; //подготавливаем идентификатор для создания ссылок на редактирование этого объекта
$id_object_encoded = urlencode($id_object); //кодируем переменную для безопасной передачи
$columnValue_encoded = urlencode($columnValue); //кодируем переменную для безопасной передачи

$DateReceipt = $row['date_of_receipt'];//сохраняем дату из бд в переменную для дальнейшего преобразования в понятный человеку формат
$DateExpiration = $row['expiration_date'];//сохраняем дату из бд в переменную для дальнейшего преобразования в понятный человеку формат
$formattedDateReceipt = date("d-m-Y", strtotime($DateReceipt));//преобразуем дату и сохраняем результат в переменную
$formattedDateExpiration = date("d-m-Y", strtotime($DateExpiration));//преобразуем дату и сохраняем результат в переменную

    echo "<tr>"; //начало строки таблицы
    echo "<td>" . $row['id'] . "</td>";//ячейка таблицы
    echo "<td>" . $row['name'] . "</td>";//ячейка таблицы
    echo "<td>" . $formattedDateReceipt . "</td>";//ячейка таблицы
    echo "<td>" . $row['cost_gram'] . "</td>";//ячейка таблицы
    echo "<td>" . $row['quant_received'] . "</td>";//ячейка таблицы
    echo "<td>" . $row['quant_spent'] . "</td>";//ячейка таблицы
    echo "<td>" . $formattedDateExpiration . "</td>";//ячейка таблицы
    echo "<td style='color: $color;'><a class='available' href='?availability=$columnValue_encoded&dop_id_object=$id_object_encoded'>$columnValue</a></td>";//цвет текста этой ячейки меняется в зависимости от значения, а само значение является ссылкой с параметром для изменения значения
...
//-------------------------------------------------Изменяем состояние объекта на основании запроса пользователя-----------------------------------------------------
// Проверяем, был ли выполнен GET-запрос
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET["availability"])) { // Проверяем, передан ли параметр "update"
       $availability = $_GET["availability"];// Передаем значение полученное из GET в переменную 
       $dop_id_object = $_GET["dop_id_object"];// Передаем значение полученное из GET в переменную
       if($availability=="Разрешен") {$updateAvailable="Запрещен";}//подготавливаем переменную для смены значения в бд 
       if($availability=="Запрещен") {$updateAvailable="Разрешен";}//подготавливаем переменную для смены значения в бд


//--------------------------------делаем запрос к бд-------------------------------------
$sql1 = "UPDATE remains_reserves
SET status = '$updateAvailable'
WHERE id = $dop_id_object";//Подготавливаем запрос по обновлению данных 
...
//---------------------------------------------------
       ...
       header("Refresh: 0; URL=/3_admin_panel/2_reserves_panel/4_remains_reserves.php");// обновляем страницу для очистки параметров get
       exit; // важно прекратить выполнение кода после вызова header()
       
    }
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
ob_end_flush(); // заканчиваем буферизацию и отправляем вывод на страницу
