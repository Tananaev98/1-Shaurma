...
$sql = "select rr.id, r.name, rr.date_of_receipt, rr.quant_received, rr.quant_spent
FROM reserves r
INNER JOIN remains_reserves rr ON r.id = rr.id_reserves
WHERE rr.quant_received - rr.quant_spent <= 0;";//Подготавливаем запрос по поиску соответсвия логина и пароля в базе. используем обратную сортировку для того что бы
