...
$sql = "select * from reserves ORDER BY availability DESC;";//Подготавливаем запрос по поиску соответсвия логина и пароля в базе. используем обратную сортировку для того что бы
...
echo "<a class='add_object' href='2.1_reserves_pattern.php'><img src='1_image/1_Add.png' class='imagesh'></a>";//выводим изображение ссылку на создание объекта
...
$columnValue = $row["availability"]; // Получаем значение второй колонки из базы данных
...
$columnValue_encoded = urlencode($columnValue); //кодируем переменную для безопасной передачи    
    ...
    echo "<td>" . $row['min_quantity'] . "</td>";//ячейка таблицы
    echo "<td>" . $row['max_quantity'] . "</td>";//ячейка таблицы
    echo "<td>" . $row['critical_quantity'] . "</td>";//ячейка таблицы
    ...
    echo "<td>" . "<a class='edit_reserv' href='3_edit_reserves.php?id_object=$id_object_encoded'>Редактировать</a>" . "</td>";//ячейка таблицы
...
SET availability = '$updateAvailable'
...
//---------------------------------------------------
       ...
       header("Refresh: 0; URL=/3_admin_panel/2_reserves_panel/2_reserves_pattern.php");// обновляем страницу для очистки параметров get
