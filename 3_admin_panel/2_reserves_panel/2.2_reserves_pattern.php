...
if (empty($_POST["min_q"])){echo "Минимальное количество не указано!"; echo "</div>"; exit();} else {$min_q=$_POST["min_q"];}//аналогично предыдущей строке

if (empty($_POST["max_q"])){echo "Максимальное количество не указано!"; echo "</div>"; exit();} else {$max_q=$_POST["max_q"];}//аналогично предыдущей строке

if (empty($_POST["crit_q"])){echo "Критическое количество не указано!"; echo "</div>"; exit();} else {$crit_q=$_POST["crit_q"];}//аналогично предыдущей строке
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------Выполняем проверку на дубль имени Шаблона запасов-----------------------------------------------------------
    $sql = "select * from reserves where name = '$name'";//Подготавливаем запрос по поиску соответсвия логина и пароля в базе. используем имя введенное пользователем
    ...
    $min_q_check = $row['min_quantity'];//сохраняем Описание из бд в переменную
    
    $max_q_check = $row['max_quantity'];//сохраняем цену из бд в переменную
    
    $crit_q_check = $row['critical_quantity'];//сохраняем ссылку на изображение из бд в переменную
    
    echo "<span class='text_yellow'>Название:</span> $name_check <br>";//выводим название найденного совпадения
    
    echo "<span class='text_yellow'>Минимальное количество:</span> $min_q_check<br>";//выводим описание найденного совпадения
    
    echo "<span class='text_yellow'>Максимальное количество:</span> $max_q_check<br>";//выводим цену найденного совпадения

    echo "<span class='text_yellow'>Критическое количество:</span> $crit_q_check<br>";//выводим цену найденного совпадения
...
$sql1 = "INSERT INTO reserves (name, min_quantity, max_quantity, critical_quantity)  
VALUES 
('$name', $min_q, '$max_q', '$crit_q')";//Подготавливаем запрос по вставке данных в таблицу
...
echo "<a href='/3_admin_panel/2_reserves_panel/2_reserves_pattern.php'>Вернуться к списку запасов</h3><br><br>";//ссылка для возврата к редактированию меню
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
pg_close($conn);//Закрываем соединение с бд
